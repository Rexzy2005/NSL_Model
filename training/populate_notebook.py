"""
populate_tflite_notebook.py
────────────────────────────
One-shot helper that writes the full 04_tflite_conversion.ipynb
to training/notebooks/04_tflite_conversion.ipynb.  Run once.
"""
import json
import pathlib

NOTEBOOK_PATH = pathlib.Path("training/notebooks/04_tflite_conversion.ipynb")


def md(text: list[str]) -> dict:
    return {"cell_type": "markdown", "metadata": {}, "source": text}


def code(text: list[str]) -> dict:
    return {
        "cell_type": "code",
...
cells = [
    ...
    md(["## Cell 2 — Mount Drive and load the Keras model"]),
    code([
    ...
    md(["## Cell 3 — Convert to float32 TFLite"]),
    code([
    ...
    md(["## Cell 4 — Convert to INT8 quantized TFLite"]),
    code([
    ...
    md(["## Cell 5 — Verify both models load and run"]),
    code([
        "print(\"=\" * 60)\n",
    ...
    md(["## Cell 6 — Accuracy comparison: float32 vs INT8"]),
    code([
    ...
    md(["## Cell 8 — Download both TFLite files"]),
    code([
